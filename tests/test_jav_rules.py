...
class JavRulesTests(unittest.TestCase):
    ...
    def test_extracts_standard_code_from_messy_title(self) -> None:
        self.assertEqual(
            extract_jav_code("[FHD] ssis 123 sample title", self.pattern),
            "SSIS-123",
        )
        self.assertEqual(
            extract_jav_code("PRWF_010-uncensored", self.pattern),
            "PRWF-010",
        )
        self.assertEqual(
            extract_jav_code("abp.987.mp4", self.pattern),
            "ABP-987",
        )
    ...
    def test_extracts_vendor_specific_codes(self) -> None:
        ...
        for title, expected in cases.items():
            with self.subTest(title=title):
                self.assertEqual(extract_jav_code(title, self.pattern), expected)
    ...
    def test_is_jav_title_uses_expanded_default_pattern(self) -> None:
        ...
        self.assertTrue(is_jav_title("1pondo_010123_001", self.pattern))
        self.assertTrue(is_jav_title("[FHD-1080] SSIS-123-C", self.pattern))
        self.assertFalse(is_jav_title("ubuntu-24.04-live-server.iso", self.pattern))
        self.assertFalse(is_jav_title("[FHD-1080] sample video.mp4", self.pattern))
